...
//登录成功，获取腾讯QQ用户信息
    public function qq($token){
        ...
        $qq   =  \think\think\ThinkOauth::getInstance('qq', $token);
        ...
        if($data['ret'] == 0){
            ...
            exception("获取腾讯QQ用户信息失败：{$data['msg']}");
        }
    }
    ...
    public function tencent($token){
        //import("ORG.ThinkSDK.ThinkOauth");
        $tencent =  \think\think\ThinkOauth::getInstance('tencent', $token);
        ...
        if($data['ret'] == 0){
            ...
            exception("获取腾讯微博用户信息失败：{$data['msg']}");
        }
    }

    //登录成功，获取新浪微博用户信息
    public function sina($token){
        $sina =  \think\think\ThinkOauth::getInstance('sina', $token);
        ...
        if($data['error_code'] == 0){
            ...
            exception("获取新浪微博用户信息失败：{$data['error']}");
        }
    }

    //登录成功，获取网易微博用户信息
    public function t163($token){
        $t163 = \think\ThinkOauth::getInstance('t163', $token);
        ...
        if($data['error_code'] == 0){
            ...
            exception("获取网易微博用户信息失败：{$data['error']}");
        }
    }

    //登录成功，获取人人网用户信息
    public function renren($token){
        $renren = \think\ThinkOauth::getInstance('renren', $token);
        ...
        if(!isset($data['error_code'])){
            ...
            exception("获取人人网用户信息失败：{$data['error_msg']}");
        }
    }

    //登录成功，获取360用户信息
    public function x360($token){
        $x360 = \think\ThinkOauth::getInstance('x360', $token);
        ...
        if($data['error_code'] == 0){
            ...
            exception("获取360用户信息失败：{$data['error']}");
        }
    }

    //登录成功，获取豆瓣用户信息
    public function douban($token){
        $douban = \think\ThinkOauth::getInstance('douban', $token);
        ...
        if(empty($data['code'])){
            ...
            exception("获取豆瓣用户信息失败：{$data['msg']}");
        }
    }

    //登录成功，获取Github用户信息
    public function github($token){
        $github = \think\ThinkOauth::getInstance('github', $token);
        ...
        if(empty($data['code'])){
            ...
            exception("获取Github用户信息失败：{$data}");
        }
    }

    //登录成功，获取Google用户信息
    public function google($token){
        $google = \think\ThinkOauth::getInstance('google', $token);
        ...
        if(!empty($data['id'])){
            ...
            exception("获取Google用户信息失败：{$data}");
        }
    }

    //登录成功，获取Google用户信息
    public function msn($token){
        $msn  = \think\ThinkOauth::getInstance('msn', $token);
        $data = $msn->call('me');
        ...
        if(!empty($data['id'])){
            ...
            exception("获取msn用户信息失败：{$data}");
        }
    }

    //登录成功，获取点点用户信息
    public function diandian($token){
        $diandian  = \think\ThinkOauth::getInstance('diandian', $token);
        ...
        if(!empty($data['meta']['status']) && $data['meta']['status'] == 200){
            ...
            exception("获取点点用户信息失败：{$data}");
        }
    }

    //登录成功，获取淘宝网用户信息
    public function taobao($token){
        $taobao = \think\ThinkOauth::getInstance('taobao', $token);
        ...
        if(!empty($data['user_buyer_get_response']['user'])){
            ...
            exception("获取淘宝网用户信息失败：{$data['error_response']['msg']}");
        }
    }

    //登录成功，获取百度用户信息
    public function baidu($token){
        $baidu = \think\ThinkOauth::getInstance('baidu', $token);
        ...
        if(!empty($data['uid'])){
            ...
            exception("获取百度用户信息失败：{$data['error_msg']}");
        }
    }

    //登录成功，获取开心网用户信息
    public function kaixin($token){
        $kaixin = \think\ThinkOauth::getInstance('kaixin', $token);
        ...
        if(!empty($data['uid'])){
            ...
            exception("获取开心网用户信息失败：{$data['error']}");
        }
    }

    //登录成功，获取搜狐用户信息
    public function sohu($token){
        $sohu = \think\ThinkOauth::getInstance('sohu', $token);
        ...
        if('success' == $data['message'] && !empty($data['data'])){
            ...
            exception("获取搜狐用户信息失败：{$data['message']}");
        }
    }
